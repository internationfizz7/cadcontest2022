 ...
 * 
 * There are only three functions: table = STR_make_stringtab(store); bool =
 * STR_present_in_stringtab(table, string, &val) insert(table, string, ptr);
 */

/*
 * 8-20-1999 - Wanda da Rosa Added include <strings.h> for IBM build
 */
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
...
HTABLE *
STR_make_stringtab(enum table_alloc we_alloc, const char *comment,
		   enum table_case ignore_case)
{
  ...
  htab = (HTABLE *) DFEF_malloc(sizeof(HTABLE), DATA_HTABLE);
  htab->size = INIT_TABLE_SIZE;
  htab->mask = htab->size - 1;
  htab->limit = (3 * htab->size) >> 2;
  htab->has = 0;
  htab->table = (ENTRY *) DFEF_calloc(1, sizeof(ENTRY) * htab->size, DATA_ENTRY);
  ...
  if (i < MAX_TABLES) {
    tables[i] = htab;
    table_names[i] = (char *) DFEF_malloc(strlen(comment) + 1, DATA_STRING);
    if (!table_names[i])
      defrNerr(37);
...
int 
STR_present_in_stringtab(void *hdl, const char *str, int **ret)
{
...
int 
STR_modify_stringtab(void *hdl, const char *str, void *new_val)
{
  HTABLE *htab = (HTABLE *) (hdl);
...
/*
 * Modified 9/3/99 -- Wanda da Rosa.  Problem with IBMRS.  For some reason
 *//* the preprocessor, changes const char* to char* */ char *
STR_make_string_in_stringtab(HTABLE * htab, const char *str, void *val)
{
  int     h;
  ...
  if (htab->table[h].string) {
	...
	return (char *) htab->table[h].string;
      }
    }
  }
...
void 
STR_add_to_stringtab(void *hdl, const char *str, void *val)
{
  HTABLE *htab = (HTABLE *) (hdl);
...
stringtab_grow(HTABLE * htab)
{
  ...
  htab->limit = (3 * htab->size) >> 2;
  htab->has = 0;
  htab->table = (ENTRY *) DFEF_calloc(1, sizeof(ENTRY) * htab->size, DATA_HTABLE);
  alloc_save = htab->alloc;
  htab->alloc = TABLE_CALLER_ALLOCS;	/* when resizing table,don't */
  for (i = 0; i < old_size; i++)
    if (old_table[i].string != NULL)
      STR_add_to_stringtab(htab, old_table[i].string,
			   old_table[i].value);
  htab->alloc = alloc_save;
  DFEF_free((char *) old_table, DATA_HTABLE);
  if (old_has != htab->has)
    defrNerr(61);
}
...
stringtab_alloc_string(HTABLE * htab, const char *str)
{
  ...
  if (need > POOL_SIZE) {
    defiError("Internal error: String too long!");
    exit(1);
  }
  if (need > htab->nleft) {
    POOL   *p = (POOL *) DFEF_malloc(sizeof(POOL), "POOL");

    /*
     * chain these together so STR_destroy_stringab can free all blocks.
...
void 
STR_destroy_stringtab(void *handle)
{
  ...
  if (i < MAX_TABLES) {
    tables[i] = 0;
    DFEF_free(table_names[i], DATA_STRING);
  }
  /* now do the freeing of the table  */
  DFEF_free((char *) htab->table, DATA_ENTRY);
  /* free all the string blocks, if allocated  */
  for (; htab->pool_base != NULL;) {
    POOL   *next = htab->pool_base->next;

    DFEF_free((char *) htab->pool_base, DATA_STRING);
    htab->pool_base = next;
  }
  DFEF_free((char *) htab, DATA_HTABLE);
}

void    STR_apply_stringtab(void *handle, void (*fn) (const char *, void *, void *), void *a) {
  HTABLE *htab = (HTABLE *) handle;
...
int 
STR_active_hashtables(const char *indent)
{
  int     i;
...
extern void *def_alias_set;	/* from lex.cpph  */

const char *
defAliasKey(int index)
{
  HTABLE *h = (HTABLE *) def_alias_set;
...
defAliasData(int index)
{
  ...
  if (h == 0)
    return 0;
  retValue = (char *) (h->table[index].value);
  return ++retValue;
}

int 
defAliasMarked(int index)
{
  const char *retValue;
...
int 
defAddAlias(const char *key, const char *data)
{
  HTABLE *h = (HTABLE *) def_alias_set;
  ...
  if (def_alias_set == 0) {
    ...
    h = (HTABLE *) def_alias_set;
  }
  STR_add_to_stringtab(def_alias_set, key, (void *) data);
  return 0;
}
