...
{
  if (!lefwIsLayerCutSpacing)
    return LEFW_BAD_ORDER;	/* has to be after *//* lefwLayerCutSpacing
				 * is called */
  fprintf(lefwFile, "\n CENTERTOCENTER");
  lefwLines++;
  lefwState = LEFW_LAYER;
 ...
 * 5.7
 */ int 
lefwLayerCutSpacingLayer(const char *name2, int stack)
{				/* optional  */
  if (!lefwIsLayerCutSpacing)
    return LEFW_BAD_ORDER;	/* has to be after *//* lefwLayerCutSpacing
				 * is called */
 ...
 * 5.7
 */ int 
lefwLayerRoutingMinimumcutWithin(double numCuts, double minWidth,
				 double cutDistance)
{
  if (!lefwFile)
    return LEFW_UNINITIALIZED;
 ...
 * 5.7
 */ int 
lefwLayerRoutingSpacingEndOfLine(double eolWidth, double eolWithin)
{
  if (!lefwFile)
    return LEFW_UNINITIALIZED;
  if (lefwState != LEFW_LAYERROUTING_START &&
 ...
 * 5.7
 */ int 
lefwLayerRoutingSpacingEOLParallel(double parSpace, double parWithin,
				   int twoEdges)
{
  if (!lefwHasLayerRoutingEol)
 ...
 * 5.7
 */ int 
lefwLayerRoutingSpacingEndOfNotchWidth(double eonWidth, double minNSpacing,
				       double minNLength)
{
  if (!lefwFile)
    return LEFW_UNINITIALIZED;
 ...
 * 5.7
 */ int 
lefwLayerRoutingMinstepMaxEdges(double distance, double maxEdges)
{
  if (!lefwFile)
    return LEFW_UNINITIALIZED;
  if (lefwState != LEFW_LAYERROUTING_START &&
...
int 
lefwLayerRoutingSpacingtableTwoWidthsWidth(double width, double runLength,
					   int numSpacing, double *spacing)
{
  int     i;
 ...
 *//* Renamed from lefwLayerAntennaCumSideDiffAreaRatio to match *//* the
    header */ int 
 ...
 * 5.7
 */ int 
lefwLayerAntennaAreaDiffReducePwl(int numPwls, double *diffAreas,
