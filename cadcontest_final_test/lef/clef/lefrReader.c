 ...
 */ void 
lefrDisableParserMsgs(int nMsg, int *msgs)
{
  int     i, j;
...
void 
lefrEnableParserMsgs(int nMsg, int *msgs)
{
  int     i, j;
...
lefrPrintUnusedCallbacks(FILE * f)
{
  ...
  if (lefrRegisterUnused == 0) {
    fprintf(f, "ERROR (LEFPARS-101): lefrSetRegisterUnusedCallbacks was not called to setup this data.\n");
    return;
  }
  for (i = 0;
       i < NOCBK;
	...
	fprintf(f, "WARNING (LEFPARS-201): LEF items that were present but ignored because of no callback:\n");
	first = 0;
      }
